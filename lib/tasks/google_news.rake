  ...
  task find_mentions: :environment do
    ...
    people.each do |person|
      ...
      rss_articles.each do |rss_article|
        ...
        next  unless current_news_source.is_local_source

        rss_article[:news_source] = current_news_source
        current_article = NewsArticle.find_or_create_by_url(rss_article.except(:summary,
                                                                               :source,
                                                                               :base_url))

        unless current_article.people.include?(person)
          mention_count += 1
          total_mentions += 1
          ...
          unless mention.save
            Rails.logger.error "news error: mention for #{current_article.title}" \
                               'did not save'
          end
        end
      ...
      unless mention_count.zero?
        Rails.logger.info "  !! #{mention_count} new mentions. !!"
      end
    end
